...
void enqueue_task(os_threadpool_t *tp, os_task_t *t)
{
	...
	// If the queue is empty, initialize the list
	if (queue_is_empty(tp))
		list_init(&tp->head);

	// Add the task to the list, and signal the threads
...
os_task_t *dequeue_task(os_threadpool_t *tp)
{
	...
	// If the queue is empty and all threads have finished their work, return NULL
	if (queue_is_empty(tp) && threads_work_done == 1)
		goto skip_dequeue;

	// If the queue is empty, wait for a signal
	while (queue_is_empty(tp) && threads_work_done == 0)
		pthread_cond_wait(&tp->condition, &tp->mutex);

	// If the queue is not empty, dequeue the task
	if (!queue_is_empty(tp)) {
...
void wait_for_completion(os_threadpool_t *tp)
{
	...
	// Signal that all threads have finished their work
	if (threads_work_done == 0)
		threads_work_done = 1;
	pthread_cond_broadcast(&tp->condition);
